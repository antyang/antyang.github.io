;(function () {
  ...
  ;(function () {
    ...
    if (!!PIBox) {
      document.getElementsByTagName('title')[0].innerText = 'About'
  ...
  ;(function () {
    var content = ''
    var proj = personalBooks
    var bookBox = document.getElementById('bookBox')
    if (!!bookBox) {
      for (var i = 0; i < proj.length; i++) {
        var tempT = '<h3>' + proj[i].name +'</h3>'
        var tempI = '<img src="' + proj[i].image + '">'
        var tempM = '<p>' + proj[i].media + '</p>'
        var tempP = '<p>' + proj[i].comment + '</p>'

        if (!proj[i].comment) {
          tempP = '<p></p>'
        }

        var temp = '<div>' + tempT + tempI + tempM + tempP + '</div>'
        content = content + temp
      }
